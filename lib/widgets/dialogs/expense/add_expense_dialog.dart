...
class AddExpenseDialog extends ConsumerStatefulWidget {
  final Future<void> Function(ExpenseCreateDTO) onExpenseAdded;
...
class _AddExpenseDialogState extends ConsumerState<AddExpenseDialog> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await widget.onExpenseAdded(dto);
      if (!mounted) return;
      setState(() {
        _feedbackError = false;
        _feedbackMessage = 'Despesa adicionada com sucesso.';
      });
      ...
      setState(() {
        ...
        _feedbackMessage = ErrorMessageUtils.fromObject(
          e,
          fallback: 'Erro ao adicionar despesa.',
        );
      });
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    final selectedFileName = _selectedFilePath?.split(RegExp(r'[\\/]')).last;
